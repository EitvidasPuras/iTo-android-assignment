...
sealed interface FeedAction {
    ...
    data class FeedSelected(val url: String) : FeedAction
...
sealed interface FeedEvent {
    data class NavigateToEntryScreen(val url: String) : FeedEvent
...
class FeedViewModel @Inject constructor(
    ...
    private suspend fun handleFeedSelection(url: String) {
        when {
            url.contains("{user}") && url.contains("{repo}") && url.contains("{category}") -> {
                /* Validate the necessary inputs */
                validateUserInput()
                validateRepoInput()
                validateCategoryInput()
                /* If any of the inputs have errors - quit */
                if (stateFlow.value.isUserError || stateFlow.value.isRepoError || stateFlow.value.isCategoryError) {
                    return
                }
                /* If the inputs are good - we happy */
                _eventFlow.emit(
                    FeedEvent.NavigateToEntryScreen(
                        url.replace("{user}", stateFlow.value.user)
                            .replace("{repo}", stateFlow.value.repo)
                            .replace("{category}", stateFlow.value.category)
                    )
                )
            }

            url.contains("{user}") && url.contains("{repo}") -> {
                ...
                validateRepoInput()
                /* If any of the inputs have errors - quit */
                if (stateFlow.value.isUserError || stateFlow.value.isRepoError) {
                    return
                }
            ...
            else -> _eventFlow.emit(FeedEvent.NavigateToEntryScreen(url))
        }
    }
